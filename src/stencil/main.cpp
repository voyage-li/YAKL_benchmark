...
int main(int argc, char **argv)
{
    ...
    {
        // stencil-2d-5
        // real2d_host a_host2d("a_host", N + 2, N + 2), b_host2d("b_host", N + 2, N + 2);
        real2d_device a_device2d("a_device2d", N + 2, N + 2), b_device2d("b_device2d", N + 2, N + 2);
        yakl::c::parallel_for(
        ...
        // stencil-2d-9
        // real2d_host a_host2d("a_host", N + 2, N + 2), b_host2d("b_host", N + 2, N + 2);
        // real2d_device a_device2d("a_device", N + 2, N + 2), b_device2d("b_device", N + 2, N + 2);
        ...
        // stencil-3d-7
        // real3d_host a_host3d("a_host3d", N + 2, N + 2, N + 2), b_host3d("b_host3d", N + 2, N + 2, N + 2);
        real3d_device a_device3d("a_device3d", N + 2, N + 2, N + 2), b_device3d("b_device3d", N + 2, N + 2, N + 2);
                    ...
                    for (int ii = -1; ii <= 1; ii++)
                        for (int jj = -1; jj <= 1; jj++)
                            for (int kk = -1; kk <= 1; kk++)
                                a(ii + 1, jj + 1, kk + 1) = a_device3d(i + ii, j + jj, k + kk);
                    b_device3d(i, j, k) =
                        randq0 * a(1, 1, 1) +
                        randq1 * a(0, 1, 1) +
                        randq2 * a(2, 1, 1) +
        ...
        // stencil-3d-27
        // real3d_host a_host3d("a_host", N + 2, N + 2, N + 2), b_host3d("b_host", N + 2, N + 2, N + 2);
        // real3d_device a_device3d("a_device", N + 2, N + 2, N + 2), b_device3d("b_device", N + 2, N + 2, N + 2);
        ...
        // stencil-1d-3
        real1d_device a_device1d("a_device1d", N + 2), b_device1d("b_device1d", N + 2);
        yakl::c::parallel_for(
            yakl::c::Bounds<1>(1, N + 1),
            YAKL_LAMBDA(int i) {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            yakl::c::parallel_for(
                yakl::c::Bounds<1>(1, N / 8 + 1),
                YAKL_LAMBDA(int i) {
                    yakl::SArray<yakl::simd::Pack<double, 8>, 1, 3> a;
                    yakl::SArray<yakl::simd::Pack<double, 8>, 1, 3> b;
                    for (int ii = -1; ii <= 1; ii++)
                    {
                        yakl::simd::iterate_over_pack([&](unsigned int ilane)
                                                      { a(ii + 1)(ilane) = a_device1d(min(N, i * 8 + ilane) + ii); }, yakl::simd::PackIterConfig<8, true>());
                    }
                    b(i) =
                        randq0 * a(1) +
                        randq1 * a(0) +
        ...
        // fdtd-2d
        {
            ...
            real2d_host ex_host("ex_host", N, N + 1), ey_host("ey_host", N + 1, N),
                hz_host("hz_host", N, N);
            real2d_device ex_device("ex_device", N, N),
                ey_device("ey_device", N, N), hz_device("hz_device", N, N);
            ...
            for (int t = 0; t < tmax; t++)
            {
                ...
                parallel_for(
                    Bounds<1>(N), YAKL_LAMBDA(int i) { ey_device(0, i) = t; });
                parallel_for(
                    Bounds<2>({0, N}, {0, N}), YAKL_LAMBDA(int i, int j) {
                        if (i > 1)
                        {
